                    ...
                    if (TimeExist == true)
                    {
...

                        {
...

                            {
                                ...
                                sqlite_cmd = sqlite_conn.CreateCommand();
                                //  sqlite_cmd.CommandText = "INSERT INTO Games (id, home_name, away_name, score,time,league_id,status)";
                                sqlite_cmd.CommandText = "INSERT INTO Players (id, date, home_name, away_name,score, sort,ht_score,ft_score,et_score,time,league_id,status,added,last_changed,home_id,away_id,competition_id,competition_name,fixture_id,scheduled) +  " +
                                    " VALUES(@id,@match_id,@player,@time,@event1,@sort,@home_away)";
                                sqlite_cmd.Parameters.AddWithValue("@id", score1.id);
